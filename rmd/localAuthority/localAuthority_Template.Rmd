...
```{r laCo2eColPlot, fig.height=6, fig.cap = "CO2e emissions by category"}
...
if(netSum > 0){
  netSumTxt <- paste0("positive, so emissions are larger than the level of sequestration (carbon capture). In this sense ", rmdParams$district, " is not yet biogenic net-zero.")
} else {
  netSumTxt <- paste0("negative, so emissions are lower than the level of sequestration (carbon capture). In this sense ", rmdParams$district, " is biogenic net negative!")
}
```

In the most recent year, `r rmdParams$district` had net biogenic emissions using the categories defined in the plot of `r round(netSum)` kT CO2e (negative emissions are `r round(pc)` % of gross emissions). This value is `r netSumTxt`
...
**Contact**: Dr [Ben Anderson](https://twitter.com/dataknut)
