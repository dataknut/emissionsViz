 ...
 * **`r params$parish`** (Local Authority district: _`r params$district`_)
...
plotDT <- parish_terr_absDT[name == params$parish]
parish_terr_sum_kT <- sum(plotDT$value)/1000
myCap <- paste0(params$parish, ": total territorial emissions = ", round(parish_terr_sum_kT,1), " kT CO2e")
...
parDT <- parish_terr_perhhDT[name == params$parish]
laDT <- la_terr_perhhDT[name == params$district]
 ...
 * `r params$parish` total per household territorial emissions = `r round(sum_p_perhh,1)` T CO2e
 * `r params$district` total per household territorial emissions = `r round(sum_d_perhh,1)` T CO2e
...
plotDT <- parish_cons_absDT[name == params$parish]
parish_cons_sum_kT <- sum(plotDT$value)/1000
pcDiff <- 100*parish_cons_sum_kT/parish_terr_sum_kT

myCap <- paste0(params$parish, ": total consumption emissions = ", round(parish_cons_sum_kT,1), " kT CO2e, ",
...
parDT <- parish_cons_perhhDT[name == params$parish, .(id, name, variable_n,  value)]
laDT <- la_cons_perhhDT[name == params$district, .(id, name, variable_n,  value)]
plotDT <- rbind(parDT, laDT)
 ...
 * `r params$parish` total per household consumption emissions = `r round(sum_p_perhh,1)` T CO2e
 * `r params$district` total per household consumption emissions = `r round(sum_d_perhh,1)` T CO2e
...
makeFlexTable(parish_terr_absDT[name == params$parish], cap = "Parish territorial emissions")
```
...
makeFlexTable(parish_cons_absDT[name == params$parish, .(id,parish, variable_n, value)], cap = "Parish consumption emissions")
```
