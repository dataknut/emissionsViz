...
# load the parish data then melt it to long form ----
#> territorial ----
...
parish_code <- dt[name == parish, id] # extract parish code for LA matching
mdt <- melt(dt, id.vars = c("id", "name"))
...
la <- lut_DT[PAR20CD == parish_code, LAD20NM]
 ...
 * `r la` total per household territorial emissions = `r round(sum_d_perhh,1)` T CO2e
 ...
 * `r la` total per household consumption emissions = `r round(sum_d_perhh,1)` T CO2e
